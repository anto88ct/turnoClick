...
@Component({
  ...
  standalone: true,
  // Aggiunto CommonModule per utilizzare *ngIf e *ngTemplateOutlet
  ...
  template: `
    <div class="fixed top-0 left-0 right-0 z-[9999] bg-slate-900 border-b border-slate-800 shadow-lg">
      <div class="flex items-center justify-between px-3 h-9 max-w-screen-2xl mx-auto">
        ...
        <a routerLink="/" class="flex items-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity">
        ...
        <button
          ...
          class="md:hidden p-1 text-slate-400 hover:text-white rounded focus:outline-none hover:bg-white/10 transition-colors"
        >
      ...
      <div
        *ngIf="isMobileMenuOpen"
        class="md:hidden absolute top-9 left-0 w-full bg-slate-900 border-b border-slate-800 shadow-xl flex flex-col p-2 gap-1"
      >
        <ng-container *ngTemplateOutlet="navLinks"></ng-container>
      </div>
    </div>
    ...
    <ng-template #navLinks>
      <a
        ...
        class="flex items-center gap-1.5 px-2.5 py-1.5 md:py-1 rounded-lg text-xs font-semibold text-slate-400
...
export class DemoSwitcherComponent {
  // Variabile per gestire lo stato del menu su mobile
